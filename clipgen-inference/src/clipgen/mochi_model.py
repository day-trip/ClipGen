...
class MochiModel(ModelInterface):
    ...
    async def generate(self, input_data: Dict[str, Any]) -> Path:
        ...
        print(f"Raw frames from pipeline: shape={frames.shape}, dtype={frames.dtype}, min={frames.min():.6f}, max={frames.max():.6f}")
        if hasattr(frames, 'isnan'):
            nan_count = frames.isnan().sum().item() if hasattr(frames.isnan().sum(), 'item') else 0
            print(f"NaN count: {nan_count}")
        print(f"Sample frame values: {frames.flatten()[:10]}")
